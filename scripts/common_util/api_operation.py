...
class API_OPERATIONS :
    ...
    def get_repo_details(self, keyword, per_page, page):
        url = self.request_url + 'search/repositories?q=' + str(keyword) + '&per_page='+ str(per_page)+'&page=' + str(page) +'&sort=stars&order=desc'
    ...
    def get_commits_details(self, git_op, keyword, per_page, page):
        url = self.request_url + 'repos/' + str(repo_name) +'/' + str(owner_name) + '/' + st(git_op) + '?per_page='+str(per_page) + '&page=' + str(page)
        response = requests.get(url)
        data = response.json()
        return data
